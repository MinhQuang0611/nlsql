  ...
  return (
    {
      [SyncStatus.IN_PROGRESS]: (
        <Space size={[4, 0]}>
          ...
          <Text className="gray-1">Đang triển khai...</Text>
        </Space>
      ),
      [SyncStatus.SYNCRONIZED]: (
        <Space size={[4, 0]}>
          <CheckCircleOutlined className="mr-1 green-7" />
          <Text className="gray-1">Đã đồng bộ</Text>
        </Space>
      ),
      [SyncStatus.UNSYNCRONIZED]: (
        <Space size={[4, 0]}>
          <WarningOutlined className="mr-1 gold-6" />
          <Text className="gray-1">Thay đổi chưa triển khai</Text>
        </Space>
      ),
    }[syncStatus] || ''
  );
...
export default function Deploy() {
  ...
  return (
    <Space size={[16, 0]}>
      ...
      <Button
        className={`text-sm adm-modeling-header-btn ${disabled ? '' : 'gray-10'}`}
        disabled={disabled}
        onClick={() => onDeploy()}
        size="small"
