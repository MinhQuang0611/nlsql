...
export default function ViewSQLTabContent(props: AnswerResultProps) {
  ...
  const onCopy = () => {
    if (!nativeSQLResult.nativeSQLMode) {
      message.success(
        <>
          Bạn đã sao chép Wren SQL. Ngôn ngữ này là cho Wren Engine
          <br />
          và có thể không chạy trực tiếp trên cơ sở dữ liệu của bạn.
          {hasNativeSQL && (
            <>
              {' '}
              Nhấn “<b>Xem SQL gốc</b>” để lấy phiên bản chạy được.
            </>
          )}
        </>,
      );
        ...
        {previewDataResult?.data?.previewData && (
          <div className="mt-2 mb-3">
            ...
            <div className="text-right">
              <Text className="text-base gray-6">Hiển thị tối đa 500 dòng</Text>
            </div>
          </div>
        )}
      </div>
