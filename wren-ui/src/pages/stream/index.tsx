...
export default function StreamPage() {
  ...
  const sendMessage = async (text?: string) => {
    ...
    const isStreamingSupported = streamingMode && selectedTables.length === 0;
    const endpoint = selectedTables.length > 0 
      ? defaultEndpoint.replace('/chat', '/chat_with_table') 
      : (isStreamingSupported ? `${defaultEndpoint}/stream` : defaultEndpoint);
